...
axdinterpolator::SeparatedPair::SeparatedPair(
    AXDSignature const &sig, z3::expr_vector const &conjunction,
    z3::expr_vector &initial_index_vars, ArrayVars const &array_var_ids)
    : sig(sig),
  ...
  for (auto const &current_conj : conjunction) {
	...
	// in the commented 'process_part_1' label
	else{
  ...
  for (auto const &equation : part_1) {
    ...
    if (f_name.find("wr") != std::string::npos) {
      write_vector.add(lhs(equation), lhs(rhs(equation)), rhs(rhs(equation)),
		       rhs(equation).arg(2));
    }
