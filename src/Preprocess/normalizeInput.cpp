...
z3::expr
axdinterpolator::Preprocessor::normalizeInputDefault(z3::expr const &e,
  ...
  for (unsigned i = 0; i < e.num_args(); ++i)
    expr_args.push_back(normalizeInput(e.arg(i), side));
  ...
  case Z3_OP_UNINTERPRETED: {
    ...
    if (f_name.range().is_array()) {
	  ...
	  func_name(e).c_str(), sort_args,
	  sig.getArraySortBySort(f_name.range().array_range()));

      return lifted_f_name(expr_args);
    } else {
      z3::func_decl lifted_f_name =
	  sig.ctx.function(func_name(e).c_str(), sort_args, f_name.range());
...
z3::expr axdinterpolator::Preprocessor::normalizeInput(z3::expr const &e,
  ...
  if (e.is_app())
	...
	case Z3_OP_EQ: // ==
	  return normalizeInput(predicate.arg(0), side) !=
		 normalizeInput(predicate.arg(1), side);
	case Z3_OP_DISTINCT: // !=
	  return normalizeInput(predicate.arg(0), side) ==
		 normalizeInput(predicate.arg(1), side);
	case Z3_OP_GE: // >=
	...
	case Z3_OP_UNINTERPRETED:
	  if (predicate.get_sort().is_bool()) {
	    return normalizeInput(predicate, side) == sig.ctx.bool_val(false);
	  }
	default:
      ...
      return normalizeInputDefault(e, side);
	...
	return curr_rd(normalizeInput(array_arg, side), index_arg);
      }
	...
	return curr_diff(normalizeInput(array_arg1, side),
			 normalizeInput(array_arg2, side));
      }
      return normalizeInputDefault(e, side);
	...
	return curr_wr(normalizeInput(array_arg, side), index_arg,
		       normalizeInput(element_arg, side));
      }
      return normalizeInputDefault(e, side);
