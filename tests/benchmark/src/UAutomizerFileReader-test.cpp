...
void UAutomizerFileReader::testAXDInterpolator() const {
  ...
  BENCHMARK_COMMAND(
      ...
      sprintf(exec_command,
              "./../../../original/bin/axd_interpolator QF_LIA %s %u 1000000;",
      ...
      if (ret != 0 && ret != 152) {
        ...
        sprintf(complain_command,
                "echo File: \"%s\" Solver Code: \"%u\" Sample Id: %d Exit "
                "Code: %d >> /home/jose/bad_cases.txt",
                file_for_implementation.c_str(), curr_solver + 3,
                500 - num_samples, ret);
      ...
      sprintf(log_command,
              ...
              file_for_implementation.c_str(), curr_solver + 3, ret,
              file_statistics););
  system(("rm -rf " + temp_file).c_str());
}

void UAutomizerFileReader::testCAXDInterpolator() const {
  ...
  BENCHMARK_COMMAND(
      ...
      sprintf(log_command,
              "echo File: \"%s\" Solver Code: \"%u\" Exit Code: %d >> \"%s\"",
              file_for_implementation.c_str(), curr_solver, ret,
              file_statistics););
  system(("rm -rf " + temp_file).c_str());
}
...
void UAutomizerFileReader::testOtherSolvers() {
  ...
  case Z3: {
    ...
    BENCHMARK_COMMAND(
        ...
        tseitin_solver.add(z3::mk_and(part_a));
        tseitin_solver.add(z3::mk_and(part_b));
        axdinterpolator_file << "(set-option :produce-interpolants true)"
                             << std::endl;
        axdinterpolator_file << "(set-logic QF_AUFLIA)" << std::endl;
        ...
        std::string line(""); std::string interpolant_from_file("");
        ...
        if (line == "unsat") {
          ...
          sprintf(
              log_command,
              "echo File: \"%s\" Solver Code: \"%u\" Exit Code: %d >> \"%s\"",
              file_for_implementation.c_str(), 6, ret, file_statistics);
        } else {
          system(("rm -rf " + temp_file_name).c_str());
          sprintf(
              log_command,
              "echo File: \"%s\" Solver Code: \"%u\" Exit Code: %d >> \"%s\"",
              file_for_implementation.c_str(), 6, 1, file_statistics);
        });
  ...
  case MATHSAT: {
    ...
    BENCHMARK_COMMAND(
        ...
        if (line == "unsat") {
          ...
          if (_interpolant_result.find("build ie-local interpolant") !=
              std::string::npos) {
            system(("rm -rf " + temp_file_name).c_str());
            sprintf(
                log_command,
                "echo File: \"%s\" Solver Code: \"%u\" Exit Code: %d >> \"%s\"",
                file_for_implementation.c_str(), 7, 1, file_statistics);
          } else {
            ...
            mathsat_interpolant_parser.from_string(
                interpolant_from_file.c_str());

            auto const &interpolant_result =
                mathsat_interpolant_parser.assertions();

            sprintf(
                log_command,
                "echo File: \"%s\" Solver Code: \"%u\" Exit Code: %d >> \"%s\"",
                file_for_implementation.c_str(), 7, ret, file_statistics);
          }
        } else {
          system(("rm -rf " + temp_file_name).c_str());
          sprintf(
  ...
  case SMTINTERPOL: {
    ...
    BENCHMARK_COMMAND(
        ...
        tseitin_solver.add(z3::mk_and(part_a));
        tseitin_solver.add(z3::mk_and(part_b));
        axdinterpolator_file << "(set-option :print-success false)\n"
        ...
        sprintf(exec_command,
                "java -jar ./../../bin/smtinterpol-2.5-1256-g55d6ba76.jar -w "
                "%s > %s",
                file_for_implementation.c_str(), temp_file_name.c_str());
        ,
        // LOG_COMMAND
        ...
        if (line == "unsat") {
          ...
          smtinterpol_interpolant_parser.from_string(
              interpolant_from_file.c_str());
          ...
          sprintf(
              log_command,
              "echo File: \"%s\" Solver Code: \"%u\" Exit Code: %d >> \"%s\"",
              file_for_implementation.c_str(), 8, ret, file_statistics);
        } else {
          system(("rm -rf " + temp_file_name).c_str());
          sprintf(
              log_command,
              "echo File: \"%s\" Solver Code: \"%u\" Exit Code: %d >> \"%s\"",
              file_for_implementation.c_str(), 8, 1, file_statistics);
        }
